...
int main(int argc, char **argv) {
    ...
    while (1) {
        ...
        if (msg.command == '1') {
			...
			int broj_poruka=0;
			char* piece = strtok(message, ":");
			
			strcpy(poruke[broj_poruka],piece);
			int postavi =0;
			...
			while(piece != NULL){
				...
				piece = strtok(NULL, ":");
					...
					for(int i=0;i<10;++i){
				for(int j=0;j<100;++j){
					if(poruke[i][j] == '\0' && postavi != 1){
						poruke[i][j]='\n';
						postavi=1;
						}						//ovdje sam probo rascjepati poruku ali mi ne radi pravilno
					else if(postavi==1){
						poruke[i][j]='\0'; //predpostavljam da ovo ne radi dobro jer prosljeduje istu memorijsku lokaciju pa prebrise sve poslije
						}
					}
				}
            ...
            for (int i = 0; i < 20; ++i) {
				...
				if(i == 0)
				err = getaddrinfo(msg.ipp[i].IP, port, &hints, &res);
